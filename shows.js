...
.then(shows => {
    ...
    const upcomingShows = shows.filter(show => new Date(show.date) >= today).sort((a, b) => new Date(a.date) - new Date(b.date));
    const pastShows = shows.filter(show => new Date(show.date) < today).sort((a, b) => new Date(b.date) - new Date(a.date));
